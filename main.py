...
for k in range (n,7):
    if k > 1:
        yCharIn= input("Yellow Tiles, like: "+ yChar + "\n")
        gCharIn= input("Green Tiles, like: " + gChar + "\n")
    else:
        yCharIn = ''
        gCharIn = ''
    ...
    #if gChar == '':
    #    gChar = gCharDefault
    (pool,confirmedChars) = formPool(wChar,yChar,gChar)

    print(pool)
    print (confirmedChars)
    newList = []
    if k > 1:
        wordList = clue(pool,confirmedChars,dict)
        ...
        for word in wordList:
            ...
            newList.append({"score": score,"guess":word})
        i=0
    ...
    if k <= 6:
        ...
        yesno = input("Is it the correct Guess? yes or no \n")
        if yesno == 'yes':
            break
        else:
